...
class KMSRSADecrypterTest {
    ...
    //        KMSRSADecrypter kmsRsaDecrypter = new KMSRSADecrypter(TEST_KEY_ID,
    // this.mockKmsClient);
    //        // This is sample data for the tests
    ...
    //        // sample decrypted response that the mock Kmsclient will return
    ...
    //                        .build();
    //        System.out.println(decryptResponse.toString());
    //        // Configure mock kmsclient to return the predefined decryptedResponse when decrypt
    //        when(mockKmsClient.decrypt(any(DecryptRequest.class))).thenReturn(decryptResponse);
    //        // calls decrypt method passing in the test data
    //        byte[] result =
    ...
    //        // capture DecryptRequest that was passed to the mock KMS
    //        ArgumentCaptor<DecryptRequest> decryptRequestArgumentCaptor =
    //                ArgumentCaptor.forClass(DecryptRequest.class);
    //        // verifies decrypt method of mock was called (set up)
    ...
    //            String TEST_KEY_ID = "mrk-1234567890abcdef0123456789abcdef0";
    //            String HEADER_TEST_KEY_ID=
    // "16af225e1f7b7e385c6769eff66926ceb850fc644494947984926f1767daf9b9";
    //            KMSRSADecrypter kmsRsaDecrypter = new KMSRSADecrypter(HEADER_TEST_KEY_ID,
    // this.mockKmsClient);
    //
    //            JWEHeader header = createHeader();
    //            JWEHeader updatedHeader = new JWEHeader.Builder(header)
    //                    .keyID(HEADER_TEST_KEY_ID) // Set the 'kid'
    //                    .build();
    //            Base64URL encryptedKey =
    //                    Base64URL.from(
    //
    ...
    //            byte[] result =
    //                    kmsRsaDecrypter.decrypt(
    //                            updatedHeader, encryptedKey, iv, cipherText, authTag,
    // AAD.compute(updatedHeader));
    //            SignedJWT signedJWT = SignedJWT.parse(new String(result, StandardCharsets.UTF_8));
    //            JWTClaimsSet claims = signedJWT.getJWTClaimsSet();
    //
    //            ArgumentCaptor<DecryptRequest> decryptRequestArgumentCaptor =
    ...
    //                    actualDecryptRequest.encryptionAlgorithmAsString());
    //            assertEquals(
    //                    "alias/" + CLIENT_JAR_KMS_ENCRYPTION_KEY_ALIAS_PRIMARY,
    //                    actualDecryptRequest.keyId());
    //            assertArrayEquals(
    //                    encryptedKey.decode(),
    // actualDecryptRequest.ciphertextBlob().asByteArray());
    //            assertEquals(11, claims.getClaims().size());
    //            assertEquals("urn:uuid:8d097496-4410-49db-acdb-ffdca993fd2f",
    // claims.getSubject());
    //            assertEquals("ipv-core-stub", claims.getIssuer());
    //        }
    //
    ...
    //                    actualDecryptRequest.encryptionAlgorithmAsString());
    //            assertEquals(
    //                    "alias/" + CLIENT_JAR_KMS_ENCRYPTION_KEY_ALIAS_SECONDARY,
    //                    actualDecryptRequest.keyId());
    //            assertArrayEquals(
    //                    encryptedKey.decode(),
    // actualDecryptRequest.ciphertextBlob().asByteArray());
    ...
    @Test
    void shouldThrowExceptionWhenBothKeyAliasesAreNotPresent() throws Exception {
        environmentVariables.set("ENV_VAR_FEATURE_FLAG_KEY_ROTATION", "true");
        ...
        // Throw exceptions for both primary and secondary key decryption attempts
        when(mockKmsClient.decrypt(any(DecryptRequest.class)))
                .thenThrow(new RuntimeException("primary key failed"))
                .thenThrow(new RuntimeException("secondary key failed"));

        // Assert that the decrypt method throws an exception
        assertThrows(
                Exception.class,
                () ->
                        kmsRsaDecrypter.decrypt(
        ...
        // Verify that decrypt was called twice (once for each key)
